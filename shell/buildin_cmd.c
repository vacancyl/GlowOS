...
// 把old_abs_path中的. .. 转换成实际路径存入new_abs_path
void wash_path(char *old_abs_path, char *new_abs_path)
{
    ...
    while (name[0])
    {
        if (!strcmp("..", name)) // 返回上级
        {
            char *slash_ptr = strrchr(new_abs_path, '/'); // 注意此时的/并没有在末尾，因为在下面的else 中才会拼接
            if (slash_ptr != new_abs_path)                // 是否位于首部 /a/b /a
                *slash_ptr = 0;
            else //  /a  /
                *(slash_ptr + 1) = 0;
        }
        else if (strcmp(".", name)) // 不是.
        {
            if (strcmp(new_abs_path, "/"))
                strcat(new_abs_path, "/"); // 拼一个/
...
void make_clear_abs_path(char *path, char *final_path)
{
    ...
    if (path[0] != '/') // 如果不是绝对路径
    {
        ...
        if (getcwd(abs_path, MAX_PATH_LEN) != NULL) // 获取当前工作目录的绝对路径
        {
    ...
    // 把path 加到工作目录的头上
    strcat(abs_path, path); // 相对路径拼接 是绝对路径直接拼接就可以
