...
struct partition
{
    ...
    char name[8];                //分区名字
    struct super_block* sb;	     //本分区 超级块 在有指针操作的情况下才会涉及到数据宽度，没有使用编译可以通过
    struct bitmap block_bitmap;  //块位图 文件系统是以多个扇区为单位读写磁盘的
...
} __attribute__ ((packed));//保证是16字节大小

//引导扇区，mbr或者ebr
struct boot_sector
{
    uint8_t other[446];	  			//446 + 64 + 2 446是拿来占位置的 引导代码
    struct partition_table_entry partition_table[4];  //分区表中4项 64字节
    uint16_t signature;				//最后的标识标志 魔数0x55 0xaa				
} __attribute__ ((packed));//512字节大小
...
{
    char name[8];		      //ata通道名称
    uint16_t port_base;	      //本通道的起始端口号命令块 通道1 0x1f0-0x1f7 
                              //通道2 0x170-0x177 控制块寄存器0x3f6 0x376 
    uint8_t  irq_no;		      //本通道所用的中断号 两个硬盘一个中断
...
extern uint8_t channel_cnt;            // 按照硬盘数计算ata通道数
